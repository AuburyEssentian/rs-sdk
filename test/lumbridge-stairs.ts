...
const BOT_NAME = process.env.BOT_NAME ?? `stairs${Math.random().toString(36).slice(2, 5)}`;

// Lumbridge Castle staircase area (inside the castle)
...
async function runTest(): Promise<boolean> {
    ...
    try {
        ...
        // Helper to find stairs/staircase/ladder
        function findStairs(pattern: RegExp): NearbyLoc | null {
            const locs = sdk.getNearbyLocs();
            return locs.find(l =>
                pattern.test(l.name) &&
                l.optionsWithIndex.length > 0
            ) || null;
        }
